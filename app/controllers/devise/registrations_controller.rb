  ...
  def update
    if resource.update_with_password(params[resource_name])
      ...
      respond_to do |format|
        ...
        format.json {
          sign_out(resource) if !(params[:token].present? && params[:token] == "9c6187851155b7c1af4d13a26ada34fd" && request.headers['user-agent'].present? && request.headers['user-agent'].match(/iPhone|iPad|BuroTex/))
